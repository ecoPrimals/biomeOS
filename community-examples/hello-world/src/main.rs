//! Main binary for hello-world primal

use hello_world_primal::*;
...
async fn main() -> Result<()> {
    ...
    tracing::info!("Starting hello-world primal");
    tracing::info!("Version: {}", env!("CARGO_PKG_VERSION"));
    tracing::info!("Listening on {}:{}", args.host, args.port);
    
    // Load configuration
    let primal_config = HelloWorldConfig::default();
    
    // Create primal instance
    let primal = HelloWorld::new(primal_config);
...
async fn start_health_server(
    primal: std::sync::Arc<HelloWorld>,
    host: &str,
    port: u16,
) -> Result<()> {
    use axum::{
    ...
    let app = Router::new()
        ...
        .route("/", get(|| async { "BiomeOS hello-world Primal" }));
