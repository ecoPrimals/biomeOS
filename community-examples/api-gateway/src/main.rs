//! Main binary for api-gateway primal
...
async fn main() -> Result<()> {
    ...
    tracing::info!("Starting api-gateway primal");
    tracing::info!("Version: {}", env!("CARGO_PKG_VERSION"));
    tracing::info!("Listening on {}:{}", args.host, args.port);
    
    // Load configuration
    let primal_config = ApiGatewayConfig::default();
    
    // Create primal instance
    let primal = ApiGateway::new(primal_config);
...
async fn start_health_server(
    primal: std::sync::Arc<ApiGateway>,
    host: &str,
    port: u16,
) -> Result<()> {
    use axum::{
    ...
    let app = Router::new()
        ...
        .route("/", get(|| async { "BiomeOS api-gateway Primal" }));
