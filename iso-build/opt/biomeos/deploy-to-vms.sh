...
if [ ! -f "$SCRIPT_DIR/vm-ips.env" ]; then
    echo "❌ VM IPs not found. Run create-test-vms.sh first."
    exit 1
fi
...
for i in {1..10}; do
    if sshpass -p biomeos ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 biomeos@$ALPHA_IP "echo 'alpha ok'" &>/dev/null; then
        echo "✅ tower-alpha SSH ready"
        break
    fi
    echo "⏳ Waiting for tower-alpha SSH (attempt $i/10)..."
    sleep 10
done

for i in {1..10}; do
    if sshpass -p biomeos ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 biomeos@$BETA_IP "echo 'beta ok'" &>/dev/null; then
        echo "✅ tower-beta SSH ready"
        break
    fi
    echo "⏳ Waiting for tower-beta SSH (attempt $i/10)..."
    sleep 10
done
...
sshpass -p biomeos scp -o StrictHostKeyChecking=no "$USB_PACKAGE" biomeos@$ALPHA_IP:/tmp/ &
sshpass -p biomeos scp -o StrictHostKeyChecking=no "$USB_PACKAGE" biomeos@$BETA_IP:/tmp/ &
wait
...
sshpass -p biomeos ssh -o StrictHostKeyChecking=no biomeos@$ALPHA_IP << 'DEPLOY_ALPHA'
set -e
cd /tmp
...
sshpass -p biomeos ssh -o StrictHostKeyChecking=no biomeos@$BETA_IP << 'DEPLOY_BETA'
set -e
cd /tmp
sudo mkdir -p /mnt/usb
sudo tar -xzf biomeos-*.tar.gz -C /mnt/usb
