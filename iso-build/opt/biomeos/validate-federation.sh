...
ALPHA_PRIMALS=$(sshpass -p biomeos ssh -o StrictHostKeyChecking=no biomeos@$ALPHA_IP \
    "pgrep -f 'nestgate|songbird|beardog|toadstool' | wc -l" || echo "0")
echo "  Primals running: $ALPHA_PRIMALS"

echo ""
echo "On tower-beta ($BETA_IP):"
BETA_PRIMALS=$(sshpass -p biomeos ssh -o StrictHostKeyChecking=no biomeos@$BETA_IP \
    "pgrep -f 'nestgate|songbird|beardog|toadstool' | wc -l" || echo "0")
...
sshpass -p biomeos ssh -o StrictHostKeyChecking=no biomeos@$ALPHA_IP \
...
sshpass -p biomeos ssh -o StrictHostKeyChecking=no biomeos@$BETA_IP \
    "sudo tail -20 /opt/biomeos/logs/primals/songbird.log 2>/dev/null | grep -i 'peer\|discover' || echo '  (No peer discovery logs yet)'"
...
if [ "$SUCCESS" = true ]; then
    ...
    echo ""
    echo "Debug:"
    echo "  ssh biomeos@$ALPHA_IP (password: biomeos)"
    echo "  ssh biomeos@$BETA_IP (password: biomeos)"
