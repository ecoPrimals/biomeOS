...
use anyhow::Result;
use biomeos_tools::{
    integration::{IntegrationConfig, get_test_coverage, run_integration_tests},
...
async fn run_all_tests(config: &IntegrationConfig) -> Result<()> {
    ...
    if coverage >= 50.0 {
        print_success(&format!(
            "🎯 SUCCESS: {:.1}% coverage achieved (target: 50%)",
            coverage
        ));
    } else {
...
async fn run_quick_check(config: &IntegrationConfig) -> Result<()> {
    ...
    biomeos_tools::execute_command("cargo", &["check", "--workspace"], Some(&workspace_path))
        .await?;
