...
use crate::{
    discover_workspace_root, execute_command, print_error, print_info, print_section, print_success,
};
...
pub async fn run_integration_tests(config: &IntegrationConfig) -> Result<()> {
    ...
    print_success(&format!(
        "All integration tests completed in {:.2}s",
...
pub async fn get_test_coverage(config: &IntegrationConfig) -> Result<f64> {
    ...
    if coverage >= target {
        ...
        print_error(&format!(
            "Coverage {coverage:.1}% below target of {target:.0}%"
        ));
    }
