...
biomeOS uses GitHub Actions with three workflows:
...
### 3. `notify-sporeprint.yml` — Sporeprint Update Trigger

**Triggers:** Push to `main` only

Sends a `repository-dispatch` event to the sporeprint repository for validation summary updates.
...
- **Standards:** `ecoPrimals/infra/wateringHole/STANDARDS_AND_EXPECTATIONS.md`
