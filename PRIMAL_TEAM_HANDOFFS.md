...
**What biomeOS Needs from petalTongue:**

1. **Live Discovery Mode (Not Showcase)**
   ```bash
   # Current behavior: Shows sandbox/tutorial data
   # Desired behavior: Discovers live primals via Songbird
   
   # Launch with:
   SHOWCASE_MODE=false bin/primals/petal-tongue
   
   # Expected: Auto-discovers via Songbird, shows real ecosystem
   ```

2. **Primal Discovery Flow:**
   ```
   petalTongue starts
   → Queries Songbird for "discovery" capability
   → Gets list of all registered primals
...
3. **Architecture Document:**
   - ✅ Created: `docs/PETALTONGUE_HUMAN_INTERFACE.md`
   - Defines: petalTongue as THE human interface
