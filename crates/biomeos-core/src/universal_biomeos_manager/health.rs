...
use biomeos_types::{HealthReport, BiomeOSConfig, Health};
...
impl UniversalBiomeOSManager {
    ...
    pub async fn get_system_health(&self) -> HealthReport {
        ...
        // Delegate to the dedicated health monitor
        let health_monitor = HealthMonitor::new((*self.config).clone());
    ...
    pub async fn check_service_health(&self, service_name: &str) -> Result<HashMap<String, serde_json::Value>> {
        ...
        let primal = primals.values().find(|p| p.name == service_name || p.id == service_name);
        ...
        if let Some(primal) = primal {
            ...
            match self.probe_endpoint(&primal.endpoint).await {
                ...
                Err(_) => {
                    ...
                    result.insert("error".to_string(), serde_json::json!("Failed to probe endpoint"));
                }
            }
        } else {
            result.insert("error".to_string(), serde_json::json!(format!("Service not found: {}", service_name)));
    ...
    pub async fn check_system_health(&self) -> Result<HashMap<String, serde_json::Value>> {
        ...
        result.insert("overall_status".to_string(), serde_json::json!(health_report.health));
        ...
        for (id, primal) in primals.iter() {
            ...
            service_health.insert(id.clone(), serde_json::json!({
                "name": primal.name,
        ...
        result.insert("system_metrics".to_string(), serde_json::json!({
            "cpu_usage": 0.0,
            "memory_usage": {
                "used_bytes": 0,
                "total_bytes": 0
    ...
    pub async fn probe_service_health(&self, service_name: &str, timeout: u64) -> Result<HashMap<String, serde_json::Value>> {
        tracing::info!("🔍 Deep probing service '{}' with {}s timeout", service_name, timeout);
        ...
        if let Some(primal) = primal {
            ...
            match self.probe_endpoint(&primal.endpoint).await {
                Ok(probe_info) => {
                    ...
                    result.insert("connectivity".to_string(), serde_json::json!({
                        "reachable": true,
                    ...
                    result.insert("performance".to_string(), serde_json::json!({
                    ...
                    // Service diagnostics
                    result.insert("diagnostics".to_string(), serde_json::json!({
    ...
    pub async fn quick_system_scan(&self) -> Result<HashMap<String, serde_json::Value>> {
        ...
        result.insert("timestamp".to_string(), serde_json::json!(chrono::Utc::now()));
        result.insert("services_scanned".to_string(), serde_json::json!(primals.len()));
        ...
        for (id, primal) in primals.iter() {
            ...
            service_status.insert(id.clone(), serde_json::json!({
                "name": primal.name,
                "status": status,
                "health": primal.health
            }));
    ...
    pub async fn comprehensive_system_scan(&self) -> Result<HashMap<String, serde_json::Value>> {
        ...
        let health_report = self.get_system_health().await;
        result.insert("system_health".to_string(), serde_json::json!(health_report));
