...
impl DeploymentMode {
    ...
    pub fn description(&self) -> String {
        match self {
            DeploymentMode::ColdSpore { media_path, persistence, .. } => {
                if *persistence {
            ...
            DeploymentMode::LiveSpore { installed_version, .. } => {
    ...
    fn is_removable_mount(device: &str, mount_point: &Path) -> bool {
        ...
        mount_str.contains("/media/") || mount_str.contains("/mnt/") || mount_str.contains("/run/media/")
    }
            ...
            for line in os_release.lines() {
                if line.starts_with("NAME=") {
                    ...
                    if name.to_lowercase().contains("mac") || name.to_lowercase().contains("darwin") {
                        ...
                        return Ok(HostOS::BSD {
                            variant: name,
                        });
                    } else {
                        return Ok(HostOS::Linux {
                            distro: name,
                        });
                    }
                }
