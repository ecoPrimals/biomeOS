...
fn test_biome_error_variants() {
    ...
    let generic_error = BiomeError::Generic {
    ...
    assert_eq!(primal_error.to_string(), "Primal error: Primal initialization failed");
    assert_eq!(primal_not_found.to_string(), "Primal not found: songbird");
    assert_eq!(security_error.to_string(), "Security error: Authentication failed");
    assert_eq!(storage_error.to_string(), "Storage error: Disk full");
    assert_eq!(io_error.to_string(), "IO error: File not found");
    assert_eq!(serialization_error.to_string(), "Serialization error: JSON parsing failed");
...
fn test_biome_error_chain() {
    ...
    let final_error = BiomeError::Generic {
        message: format!("Operation failed: {}", wrapped_error),
    };
...
fn test_biome_error_display_consistency() {
    let errors = vec![
        BiomeError::ConfigError { message: "Config".to_string() },
        BiomeError::NetworkError { message: "Network".to_string() },
        BiomeError::PrimalError { message: "Primal".to_string() },
        BiomeError::PrimalNotFound("test".to_string()),
        BiomeError::SecurityError { message: "Security".to_string() },
        BiomeError::StorageError { message: "Storage".to_string() },
        BiomeError::IoError { message: "IO".to_string() },
        BiomeError::SerializationError { message: "Serialization".to_string() },
        BiomeError::Generic { message: "Generic".to_string() },
    ];
