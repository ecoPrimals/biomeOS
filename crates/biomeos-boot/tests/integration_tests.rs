...
fn setup_integration_env() -> Result<TempDir> {
    ...
    fs::write(root.join("target/release/biomeos-init"), "#!/bin/sh\necho 'init'")?;
...
fn test_kernel_manager_with_custom_kernel() -> Result<()> {
    ...
    assert!(manager.initramfs_path().to_string_lossy().contains("initramfs"));
...
fn test_concurrent_initramfs_builds() -> Result<()> {
    ...
    let handles: Vec<_> = (0..3).map(|i| {
        ...
        thread::spawn(move || -> Result<()> {
            let mut builder = InitramfsBuilder::new(root.join(format!("build_concurrent_{}", i)))?;
            builder.create_directory_structure()?;
