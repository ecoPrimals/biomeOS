...
fn test_biomeos_mkboot_initramfs_build() -> Result<()> {
    ...
    fs::write(project_root.join("target/release/biomeos-init"), "#!/bin/sh\necho init")?;
    fs::write(project_root.join("target/release/biome"), "#!/bin/sh\necho cli")?;
...
fn test_biomeos_mkboot_custom_project_root() -> Result<()> {
    ...
    let mut cmd = Command::cargo_bin("biomeos-mkboot")?;
    cmd.arg("--project-root").arg(&custom_root)
        .arg("--help");
...
fn test_biomeos_mkboot_nonexistent_project_root() -> Result<()> {
    let mut cmd = Command::cargo_bin("biomeos-mkboot")?;
    cmd.arg("--project-root").arg("/nonexistent/path/that/should/fail")
        .arg("initramfs")
        .arg("--output").arg("/tmp/test.img");
