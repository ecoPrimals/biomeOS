...
use biomeos_spore::manifest::{BinaryManifest, ManifestMeta, BinaryInfo, CompatibilityInfo};
...
async fn test_e2e_verify_fresh_spore() {
    ...
    fs::write(nucleus_path.join("tower/tower"), tower_content).await.unwrap();
    ...
    binaries.insert("tower".to_string(), BinaryInfo {
    ...
    fs::write(nucleus_path.join("MANIFEST.toml"), toml::to_string_pretty(&manifest).unwrap()).await.unwrap();
    ...
    fs::write(spore_path.join("bin/tower"), tower_content).await.unwrap();
...
"#;
    fs::write(spore_path.join("tower.toml"), tower_toml).await.unwrap();
...
async fn test_e2e_detect_and_refresh_stale() {
    ...
    fs::write(nucleus_path.join("tower/tower"), new_content).await.unwrap();
    ...
    fs::write(spore_path.join("bin/tower"), old_content).await.unwrap();
...
async fn test_e2e_multi_binary_refresh() {
    ...
    fs::write(nucleus_path.join("tower/tower"), tower_content).await.unwrap();
    fs::write(nucleus_path.join("primals/beardog-server"), beardog_content).await.unwrap();
    fs::write(nucleus_path.join("primals/songbird-orchestrator"), songbird_content).await.unwrap();
    ...
    for (name, content, path) in [
        ...
        ("songbird-orchestrator", songbird_content, "primals/songbird-orchestrator"),
    ] {
        let mut hasher = Sha256::new();
        hasher.update(content);
        let sha256 = format!("{:x}", hasher.finalize());
        
        binaries.insert(name.to_string(), BinaryInfo {
            name: name.to_string(),
            version: "1.0.0".to_string(),
            git_commit: "abc123".to_string(),
            build_date: chrono::Utc::now(),
    ...
    let spore_path = temp_dir.path().join("spore/biomeOS");
    fs::create_dir_all(spore_path.join("bin")).await.unwrap();
    fs::create_dir_all(spore_path.join("primals")).await.unwrap();
    
    fs::write(spore_path.join("bin/tower"), b"old tower").await.unwrap(); // Stale
    fs::write(spore_path.join("primals/beardog-server"), beardog_content).await.unwrap(); // Fresh
    fs::write(spore_path.join("primals/songbird-orchestrator"), b"old songbird").await.unwrap(); // Stale
...
async fn test_e2e_verify_all_spores() {
    ...
    for i in 1..=3 {
        ...
        fs::create_dir_all(spore_path.join("bin")).await.unwrap();
        fs::write(spore_path.join("bin/tower"), b"tower").await.unwrap();
        
        let tower_toml = format!(r#"
[meta]
family_id = "test-family"
node_id = "test-node-{}"
...
async fn test_e2e_manifest_generation() {
    ...
    fs::create_dir_all(nucleus_path.join("primals")).await.unwrap();
    fs::write(nucleus_path.join("tower/tower"), b"tower").await.unwrap();
    fs::write(nucleus_path.join("primals/beardog-server"), b"beardog").await.unwrap();
