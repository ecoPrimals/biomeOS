...
async fn test_verifier_from_nucleus_with_manifest() {
    ...
    fs::write(nucleus_path.join("MANIFEST.toml"), manifest_content).await.unwrap();
...
async fn test_verifier_from_nucleus_without_manifest() {
    ...
    fs::write(nucleus_path.join("tower/tower"), b"tower binary").await.unwrap();
    fs::write(nucleus_path.join("primals/beardog-server"), b"beardog binary").await.unwrap();
