...
fn test_missing_beardog_binary() {
    ...
    fixture.write_file("biomeOS/tower.toml", r#"
[tower]
family_id = "test"
node_id = "test-node"
"#);
...
fn test_wrong_architecture_binary() {
    ...
    std::fs::write(fixture.spore_path.join("biomeOS/primals/beardog"), &elf_header).ok();
...
"#);
    ...
    assert!(tower.get("family_id").is_none(), "family_id should be missing");
...
"#);
    ...
    assert!(!songbird_marker.exists(), "Songbird marker should not exist");
}
