...
use biomeos_spore::incubation::{LocalEntropy, SporeIncubator, IncubatedNode};
...
fn test_local_entropy_generation() {
    ...
    assert!(!entropy.machine_id.is_empty(), "Machine ID should not be empty");
...
fn test_entropy_hash_deterministic() {
    ...
    assert_eq!(hash1, hash2, "Hash should be deterministic for same entropy");
}
...
fn test_entropy_hash_unique() {
    ...
    assert_ne!(hash1, hash2, "Different hostnames should produce different hashes");
}
...
fn test_entropy_with_optional_fields_none() {
    ...
    let hash = entropy.hash();
    assert_eq!(hash.len(), 64, "Hash should work with missing optional fields");
}
