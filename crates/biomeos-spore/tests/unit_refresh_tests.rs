...
async fn test_refresh_spore_with_stale_binary() {
    ...
    fs::write(nucleus_path.join("tower/tower"), fresh_content).await.unwrap();
    ...
    fs::write(spore_path.join("bin/tower"), b"old tower v1").await.unwrap();
...
async fn test_refresh_spore_already_fresh() {
    ...
    fs::write(nucleus_path.join("tower/tower"), content).await.unwrap();
    ...
    let spore_path = temp_dir.path().join("spore1/biomeOS");
    fs::create_dir_all(spore_path.join("bin")).await.unwrap();
    fs::write(spore_path.join("bin/tower"), content).await.unwrap();
...
async fn test_refresher_dry_run_doesnt_modify() {
    ...
    fs::write(spore_path.join("primals/beardog-server"), old_content).await.unwrap();
