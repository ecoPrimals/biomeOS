...
"#,
    ...
    /// 
    /// Copies ALL binaries from plasmidBin/:
...
"#,
    ...
    async fn create_spore_manifest(&self) -> SporeResult<()> {
        ...
        let mut manifest = SporeManifest::new(
            ...
            "nat0".to_string(), // TODO: Get from config
        ...
        if nucleus_path.exists() {
            match BinaryManifest::from_nucleus(&nucleus_path) {
                Ok(nucleus_manifest) => {
                    ...
                    for (key, binary) in nucleus_manifest.binaries {
                        manifest.add_binary(
                            binary.name.clone(),
                    ...
                    info!("Added {} binaries to spore manifest", manifest.binaries.len());
                }
                Err(e) => {
