...
impl FossilIndex {
    ...
    pub fn find_by_node(&self, node_id: &str) -> Vec<&FossilIndexEntry> {
        self.fossils.iter().filter(|f| f.node_id == node_id).collect()
    }
...
impl LogManager {
            ...
            if path.is_dir() {
                ...
                if metadata_path.exists() {
                    match self.load_active_session(&metadata_path) {
                        ...
                        Err(e) => {
                            warn!("Failed to load session from {}: {}", metadata_path.display(), e);
                        }
                    }
                }
            }
    ...
    pub async fn archive_session(
        ...
        for log_file in &session.log_files {
            if log_file.path.exists() {
                ...
                debug!("Archived log: {} → {}", log_file.path.display(), dest.display());
            }
        }
    ...
    async fn update_fossil_index(&self, fossil: &FossilRecord, fossil_path: &PathBuf) -> SporeResult<()> {
    ...
    pub async fn cleanup_stale_sessions(&self) -> SporeResult<Vec<FossilRecord>> {
        ...
        for session in active_sessions {
            if !session.is_active() {
                ...
                let fossil = self.archive_session(&session, ArchivalReason::AutomaticRotation).await?;
...
impl SporeLogManager {
    ...
    pub async fn initialize(&self) -> SporeResult<()> {
        ...
        info!("✅ Spore log directories initialized: {}", log_dir.display());
        Ok(())
    }
