...
impl PrimalDiscovery {
    ...
    pub async fn discover_all(&self) -> Result<Vec<DiscoveredPrimal>> {
        ...
        let mut entries = tokio::fs::read_dir(&self.runtime_dir)
            .await
            .with_context(|| format!("Failed to read runtime directory: {}", self.runtime_dir.display()))?;
...
mod tests {
    ...
    fn test_extract_family_id() {
        ...
        assert_eq!(
            DiscoveredPrimal::extract_family_id("beardog.sock"),
            None
        );
    }
