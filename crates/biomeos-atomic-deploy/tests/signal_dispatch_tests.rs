...
//! signals to graph paths, loads all 14 signal graphs, and intercepts
...
#[test]
fn list_signal_graphs_finds_all_14() {
    ...
    assert_eq!(
        signals.len(),
        14,
        "Expected 14 signal graphs, found {}",
        signals.len()
    );
...
fn signal_schema_loads() {
    ...
    assert_eq!(tools_arr.len(), 14, "Expected 14 tool definitions");
}
