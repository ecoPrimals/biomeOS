...
async fn json_rpc_call(socket_path: &PathBuf, method: &str, params: serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    tokio::time::timeout(Duration::from_secs(10), buf_reader.read_line(&mut response_line))
        .await
...
async fn test_nucleation_xdg_paths() {
    ...
    assert!(
        beardog_socket.to_string_lossy().contains("/run/user/1000/biomeos/"),
        "BearDog socket should be in XDG runtime dir: {:?}",
        beardog_socket
    );
    assert!(
        songbird_socket.to_string_lossy().contains("/run/user/1000/biomeos/"),
    ...
    assert_eq!(beardog_socket, beardog_socket_2, "Same primal should get same socket");
}
...
async fn test_nucleation_batch_assignment() {
    ...
    for (primal, path) in &assignments {
        assert!(
            path.to_string_lossy().contains(&format!("{}-batch-test.sock", primal)),
...
async fn test_execution_context_socket_paths() {
    ...
    let nucleation = Arc::new(RwLock::new(SocketNucleation::new(SocketStrategy::XdgRuntime)));
