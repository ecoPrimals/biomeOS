...
async fn test_binary_tree_structure() {
    ...
    let node_count = root.get_node_count().await.expect("Failed to get node count");
...
async fn test_workload_submission() {
    ...
    let workload_id = root.submit_workload(workload)
        .await
        .expect("Failed to submit workload");
    
    // Verify it was submitted
    let status = root.get_workload_status(&workload_id)
        .await
...
async fn test_isomorphic_interface() {
    ...
    for child in children {
        ...
        // Grandchildren too
        let grandchildren = child.get_children().await.expect("Failed to get grandchildren");
...
async fn test_metrics_aggregation() {
    ...
    for i in 0..4 {
        let workload = Workload::builder(format!("workload-{}", i), Runtime::Native)
            .cpu_cores(1)
            .build();
        root.submit_workload(workload).await.expect("Failed to submit workload");
    }
    
    // Get subtree metrics
    let metrics = root.get_subtree_metrics().await.expect("Failed to get metrics");
...
async fn test_n_ary_tree() {
    ...
    let root = FractalBuilder::new("quad-root")
        .topology(NodeTopology::NAryTree { branching_factor: 4 })
        .depth(2)
        .resources(ResourceInfo {
            cpu_cores: 16,
            memory_mb: 16384,
...
async fn test_weighted_resource_allocation() {
    ...
    let child1_resources = children[0].get_resources().await.expect("Failed to get child1 resources");
    assert_eq!(child1_resources.cpu_cores, 12);
    
    // Second child should have ~4 cores (25% of 16)
    let child2_resources = children[1].get_resources().await.expect("Failed to get child2 resources");
...
async fn test_leaf_node_direct() {
    ...
    let leaf: std::sync::Arc<dyn ComputeNode> = std::sync::Arc::new(LeafNode::new(config, resources.clone()));
    ...
    let id = leaf.submit_workload(workload).await.expect("Failed to submit to leaf");
    
    // Check status
    let status = leaf.get_workload_status(&id).await.expect("Failed to get status");
    assert_eq!(status, WorkloadStatus::Running);
}
