...
fn get_genome_bin_path() -> Option<PathBuf> {
    ...
    let search_paths = [
        ...
        "/home/eastgate/Development/ecoPrimals/wateringHole/genomeBin", // Absolute fallback
    ];
...
pub async fn get_manifest() -> Result<Json<DistManifest>, (StatusCode, Json<DistError>)> {
    ...
    let genome_bin = get_genome_bin_path().ok_or_else(|| {
...
mod tests {
    ...
    static GENOMEBIN_PATH_LOCK: Mutex<()> = Mutex::new(());
