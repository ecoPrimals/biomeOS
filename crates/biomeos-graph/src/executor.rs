...
// Types now imported from executor modules (no duplication)
...
impl GraphExecutor {
    ...
    pub async fn execute(&mut self) -> Result<ExecutionReport> {
        ...
        // Topological sort to get execution phases (using TopologicalSorter)
        ...
        for (phase_num, phase_nodes) in phases.iter().enumerate() {
            ...
            match self.execute_phase(phase_nodes).await {
                ...
                Err(e) => {
                    ...
                    // Rollback if enabled (using RollbackManager)
