...
use crate::graph::{PrimalGraph, GraphNode, PrimalSelector, Operation};
...
impl AiGraphAdvisor {
    ...
    fn detect_missing_error_handling(&self, graph: &PrimalGraph) -> Option<AiSuggestion> {
        ...
        let has_retry = graph.nodes.iter().any(|n| n.constraints.is_some());
...
mod tests {
    use super::*;
    use crate::graph::{GraphId, GraphEdge, EdgeType, CoordinationPattern};
    ...
    fn test_detect_parallelization() {
        ...
        assert_eq!(suggestion.suggestion_type, SuggestionType::PerformanceImprovement);
