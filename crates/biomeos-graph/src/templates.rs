...
use crate::graph::{PrimalGraph, GraphId};
...
impl GraphTemplateManager {
    ...
    pub async fn discover_nestgate(&mut self) -> Result<()> {
        ...
        match NestGateTemplateClient::discover().await {
            ...
            Err(e) => {
                tracing::warn!("⚠️  NestGate not available, templates will be memory-only: {}", e);
    ...
    pub async fn save_template(&self, template: GraphTemplate) -> Result<()> {
        // Update cache
        self.cache.write().await.insert(template.id.clone(), template.clone());
        ...
        if let Some(ref client) = self.nestgate_client {
            ...
            tracing::warn!("⚠️  Template '{}' saved to memory only (NestGate unavailable)", template.name);
        }
    ...
    pub async fn load_template(&self, template_id: &str) -> Result<GraphTemplate> {
        ...
        if let Some(ref client) = self.nestgate_client {
            ...
            // Update cache
            self.cache.write().await.insert(template_id.to_string(), template.clone());
...
mod tests {
    use super::*;
    use crate::graph::{GraphNode, PrimalSelector, Operation, NodeConstraints, RetryPolicy, CoordinationPattern};
    use chrono::Utc;
    ...
    async fn test_instantiate_template() {
        ...
        let graph = manager.instantiate_template(&template.id, HashMap::new()).await.unwrap();
    ...
    async fn test_template_with_parameters() {
        ...
        assert!(manager.instantiate_template(&template.id, HashMap::new()).await.is_err());
        
        // Instantiate with required parameter
        let mut params = HashMap::new();
        params.insert("storage_key".to_string(), serde_json::json!("my_key"));
        
        let graph = manager.instantiate_template(&template.id, params).await.unwrap();
