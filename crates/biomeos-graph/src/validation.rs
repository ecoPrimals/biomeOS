...
impl EnhancedGraphValidator {
    ...
    fn validate_structure(&self, graph: &PrimalGraph, report: &mut ValidationReport) -> Result<()> {
        ...
        if graph.nodes.len() > 1000 {
            report.add_warning(ValidationWarning::new(
                "LARGE_GRAPH",
                format!("Graph has {} nodes, consider breaking into sub-graphs", graph.nodes.len())
            ));
        }
    ...
    fn validate_primal_selector(&self, selector: &PrimalSelector, node_id: &str, report: &mut ValidationReport) {
        match selector {
    ...
    fn add_performance_suggestions(&self, graph: &PrimalGraph, report: &mut ValidationReport) {
        ...
        let nodes_with_deps: HashSet<_> = graph.edges.iter()
            .map(|e| e.to.as_str())
            .collect();
        
        let independent_nodes: Vec<_> = graph.nodes.iter()
        ...
        if independent_nodes.len() > 1 {
            report.add_suggestion(ValidationSuggestion::new(
                "PARALLELIZATION",
                format!("{} nodes have no dependencies and could be executed in parallel", independent_nodes.len())
            ));
        }
        ...
        if max_chain_length > 10 {
            report.add_suggestion(ValidationSuggestion::new(
                "PIPELINE",
                format!("Graph has a chain of {} nodes, consider pipeline pattern", max_chain_length)
            ));
        }
    }
...
mod tests {
    use super::*;
    use crate::graph::{GraphId, GraphNode, GraphEdge, CoordinationPattern, Operation, EdgeType};
    ...
    fn test_invalid_edge_reference() {
        ...
        assert!(!report.valid);
        assert!(report.errors.iter().any(|e| e.code == "INVALID_EDGE_TARGET"));
    }
    ...
    fn test_parallelization_suggestion() {
        ...
        assert!(report.suggestions.iter().any(|s| s.category == "PARALLELIZATION"));
    }
