...
impl GraphParser {
    ...
    pub fn parse_file(path: &Path) -> Result<PrimalGraph> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| GraphError::ParseError(format!("Failed to read {}: {}", path.display(), e)))?;
    ...
    pub fn parse_toml(content: &str) -> Result<PrimalGraph> {
        ...
        let version = Self::get_optional_string(graph_table, "version").unwrap_or_else(|| "1.0.0".to_string());
    ...
    fn parse_coordination(table: &toml::map::Map<String, toml::Value>) -> Result<CoordinationPattern> {
        ...
        match coord_str.as_str() {
            ...
            other => Err(GraphError::ParseError(format!("Unknown coordination pattern: {}", other))),
        }
    }
    ...
    fn parse_nodes(value: &toml::Value) -> Result<Vec<GraphNode>> {
        ...
        if nodes_array.is_empty() {
            return Err(GraphError::ParseError("Graph must have at least one node".to_string()));
        }
    ...
    fn parse_node(value: &toml::Value) -> Result<GraphNode> {
        ...
        let output = Self::get_optional_string(table, "output");
        let parallel_group = table.get("parallel_group")
    ...
    fn parse_primal_selector(table: &toml::map::Map<String, toml::Value>) -> Result<PrimalSelector> {
        ...
        let primal_table = primal_value.as_table()
        ...
        if let Some(caps_value) = primal_table.get("by_capabilities") {
            if let Some(caps_array) = caps_value.as_array() {
                let caps: Result<Vec<String>> = caps_array.iter()
                    .map(|v| v.as_str()
                        .ok_or_else(|| GraphError::ParseError("by_capabilities must be array of strings".to_string()))
                        .map(|s| s.to_string()))
                    .collect();
                return Ok(PrimalSelector::ByCapabilities { by_capabilities: caps? });
            }
        }
        
        Err(GraphError::ParseError("primal selector must have by_id, by_capability, or by_capabilities".to_string()))
    }
    ...
    fn parse_operation(table: &toml::map::Map<String, toml::Value>) -> Result<Operation> {
        ...
        let op_table = op_value.as_table()
            .ok_or_else(|| GraphError::ParseError("node.operation must be a table".to_string()))?;
        
        let name = Self::get_string(op_table, "name")?;
        
        let params = op_table.get("params")
            .cloned()
            .unwrap_or(toml::Value::Table(toml::map::Map::new()));
        
        // Convert TOML value to JSON value
        let params_json = serde_json::to_value(&params)
            .map_err(|e| GraphError::ParseError(format!("Failed to convert params to JSON: {}", e)))?;
    ...
    fn parse_constraints(value: &toml::Value) -> Result<NodeConstraints> {
        ...
        let timeout_ms = table.get("timeout_ms")
            .and_then(|v| v.as_integer())
            .map(|i| i as u64);
        
        let retry = table.get("retry")
            .map(|v| Self::parse_retry_policy(v))
            .transpose()?;
        
        Ok(NodeConstraints { timeout_ms, retry })
    }
        ...
        let edge_type = if let Some(et_value) = table.get("edge_type") {
            if let Some(s) = et_value.as_str() {
                if s == "dependency" {
                    ...
                    EdgeType::DataFlow { data_flow: s.to_string() }
                }
            } else if let Some(et_table) = et_value.as_table() {
                if let Some(df) = Self::get_optional_string(et_table, "data_flow") {
                    EdgeType::DataFlow { data_flow: df }
                } else {
                    EdgeType::Dependency
                }
            } else {
...
"#;
        ...
        assert_eq!(graph.nodes[0].parallel_group, Some(1));
    }
