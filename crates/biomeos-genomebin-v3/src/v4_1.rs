...
impl GenomeBin {
    ...
    pub fn write_v4_1(
        &self,
        output: &Path,
        extractors: &HashMap<Arch, &Path>,
    ) -> Result<()> {
        tracing::info!("Creating genomeBin v4.1 (multi-arch fat binary)...");
        
        let mut file = File::create(output)
            .context("Failed to create genomeBin v4.1 file")?;
        ...
        tracing::info!("   Bootstrap selector: {} bytes (padded to {})", 
        ...
        for (arch, extractor_path) in sorted_extractors {
            ...
            tracing::info!("   {} extractor: {} bytes (padded to {})", 
                arch_str, actual_size, EXTRACTOR_SIZE);
        }
        ...
        tracing::info!("   Extractor table: {} entries ({} bytes)", 
            extractors.len(), TABLE_SIZE);
        ...
        tracing::info!("   Extractors: {}KB ({} architectures)", 
            extractor_data.len() / 1024, extractors.len());
        tracing::info!("   Payload: {}KB", 
            (total_size - payload_offset) / 1024);
    ...
    fn write_v4_payload(&self, file: &mut File) -> Result<()> {
        ...
        let manifest_compressed = zstd::encode_all(&manifest_json[..], 19)
            .context("Failed to compress manifest")?;
        
        tracing::debug!("   Manifest: {} → {} bytes (compressed)", 
        ...
        for (arch, compressed_bin) in &sorted_binaries {
            ...
            entry.extend_from_slice(&(compressed_bin.size_original as u32).to_le_bytes());  // uncompressed_size (4 bytes)
...
mod tests {
    ...
    fn test_extractor_entry_size() {
        // Verify entry is exactly 32 bytes (alignment requirement)
        let entry = ExtractorEntry::new("x86_64", 0, 0, &[0u8; 32]);
        let bytes = entry.to_bytes();
        assert_eq!(bytes.len(), 32, "ExtractorEntry must be exactly 32 bytes");
    }
    ...
    fn test_bootstrap_size() {
        ...
        assert!(bootstrap.len() < BOOTSTRAP_SIZE, 
    ...
    fn test_table_capacity() {
        ...
        assert!(TABLE_SIZE >= MIN_ARCHITECTURES * 32,
