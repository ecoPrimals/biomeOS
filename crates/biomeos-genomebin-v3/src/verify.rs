...
impl GenomeBin {
    ...
    pub fn verify_all(&self) -> Result<HashMap<String, VerifyResult>> {
        ...
        for (arch, compressed) in &self.binaries {
            ...
            match compressed.decompress() {
                Ok(_) => {
                    ...
                    results.insert(arch_name.clone(), VerifyResult {
        ...
        for (i, embedded) in self.embedded_genomes.iter().enumerate() {
            ...
            let embedded_results = embedded.verify_all()
                .with_context(|| format!("Failed to verify embedded genome: {}", embedded.manifest.name))?;
        ...
        if valid_count == total_count {
            ...
            tracing::error!("❌ Verification failed: {}/{} valid", valid_count, total_count);
        }
