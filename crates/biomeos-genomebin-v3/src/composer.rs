...
impl GenomeBinComposer {
    ...
    pub fn build(self) -> Result<GenomeBin> {
        tracing::info!("Composing genomeBin: {} ({})", 
            self.name, 
        ...
        for genome in self.genomes {
            composed.embed(genome)
                .context("Failed to embed genome")?;
        }
    ...
    fn validate_atomic_composition(&self, atomic_type: &str) -> Result<()> {
        match atomic_type {
            ...
            "NODE" => {
                ...
                let has_tower = self.genomes.iter()
                    .any(|g| g.manifest.name == "tower");
                let has_components = self.genomes.iter()
                    .any(|g| g.manifest.name == "beardog") &&
                    self.genomes.iter().any(|g| g.manifest.name == "songbird");
                let has_toadstool = self.genomes.iter()
                    .any(|g| g.manifest.name == "toadstool");
                ...
                if !has_tower && !has_components {
                    anyhow::bail!("NODE atomic requires TOWER (or its components: beardog + songbird)");
                }
            ...
            "NEST" => {
                ...
                let has_components = self.genomes.iter()
                    .any(|g| g.manifest.name == "beardog") &&
                    self.genomes.iter().any(|g| g.manifest.name == "songbird");
                let has_nestgate = self.genomes.iter()
                    .any(|g| g.manifest.name == "nestgate");
...
mod tests {
    ...
    fn create_test_genome(name: &str) -> GenomeBin {
        ...
        genome.manifest = GenomeManifest::new(name)
            .version("1.0.0");
        genome
    }
    ...
    fn test_composer_no_genomes() {
        let result = GenomeBinComposer::new("empty")
            .build();
