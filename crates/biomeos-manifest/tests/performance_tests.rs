...
fn recursive_deployment_performance() {
    ...
    assert!(
        serialization_time < Duration::from_secs(2),
    ...
    assert!(
        deserialization_time < Duration::from_secs(3),
...
fn songbird_orchestration_performance() {
    ...
    let creation_time = start.elapsed();
    assert!(
        creation_time < Duration::from_millis(500),
        "Songbird orchestration creation too slow: {:?}",
        creation_time
    );
...
fn nestgate_storage_performance() {
    ...
    let creation_time = start.elapsed();
    assert!(
        creation_time < Duration::from_millis(300),
        "NestGate storage creation too slow: {:?}",
        creation_time
    );
    ...
    assert!(
        storage_validation_time < Duration::from_millis(150),
        "Storage validation too slow: {:?}",
        storage_validation_time
    );
    ...
    assert!(
        replication_time < Duration::from_millis(100),
        "Replication processing too slow: {:?}",
        replication_time
    );
...
fn template_generation_performance() {
    ...
    println!(
        ...
        template_count, generation_time
    );
    println!("  Validation: {:?}", validation_time);
    println!(
        "  Average per template: {:?}",
        generation_time / template_count as u32
    );
}
...
fn scaling_configuration_performance() {
    ...
    let creation_time = start.elapsed();
    assert!(
        creation_time < Duration::from_millis(400),
        "Scaling scenario creation too slow: {:?}",
        creation_time
    );
    ...
    assert!(
        action_time < Duration::from_millis(200),
        "Scaling action processing too slow: {:?}",
        action_time
    );
...
fn memory_usage_performance() {
    ...
    assert!(
        memory_cleanup > memory_increase / 2,
...
fn process_iterative_patterns_performance(
    iterative: &HashMap<String, IterativeDeployment>,
) -> Result<(), String> {
