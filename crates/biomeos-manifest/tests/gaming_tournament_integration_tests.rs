...
"#;
    ...
    assert_eq!(
        manifest.metadata.specialization,
        Some(BiomeSpecialization::GamingServer)
    );
...
fn test_iterative_ring_formation() {
        ...
        IterativeDeployment {
            ...
            iteration: IterationConfig {
                variables: {
                    ...
                    vars.insert(
                        "next_peer".to_string(),
                        "{{ (index + 1) % ring_size }}".to_string(),
                    );
                    vars.insert(
                        "prev_peer".to_string(),
                        "{{ (index - 1 + ring_size) % ring_size }}".to_string(),
                    );
                    vars.insert("region".to_string(), "{{ regions[index] }}".to_string());
                    vars.insert(
                        "datacenter".to_string(),
                        "tournament-{{ region }}".to_string(),
                    );
                    vars
                },
    ...
    assert_eq!(
        ring_deployment.configuration.get("ring_size").unwrap(),
        &json!(3)
    );
    assert_eq!(
        ring_deployment.configuration.get("redundancy").unwrap(),
        &json!(2)
    );
    assert_eq!(
        ring_deployment
...
fn test_tournament_scheduling() {
    ...
    manifest.schedules.insert(
        "tournament-active".to_string(),
        ScheduleConfig {
...
fn test_tournament_environments() {
    ...
    assert_eq!(
        production.environment.get("TOURNAMENT_ENV"),
        Some(&"production".to_string())
    );
    assert_eq!(
        staging.environment.get("TOURNAMENT_ENV"),
        Some(&"staging".to_string())
    );
    assert_eq!(
        development.environment.get("TOURNAMENT_ENV"),
    ...
    assert_eq!(
        prod_mycorrhiza.enforcement.threat_response,
...
fn test_complete_tournament_serialization() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse gaming tournament template: {:?}",
        result.err()
    );
    ...
    let deserialized: BiomeManifest =
        serde_yaml::from_str(&serialized).expect("Failed to deserialize manifest");

    assert_eq!(manifest.metadata.name, deserialized.metadata.name);
    assert_eq!(
        manifest.metadata.specialization,
        deserialized.metadata.specialization
    );
}
