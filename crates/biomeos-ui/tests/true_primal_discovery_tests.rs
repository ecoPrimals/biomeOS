...
async fn test_query_identity_no_socket() {
    ...
    assert_eq!(identity, "unknown", "Should return 'unknown' for unreachable primal");
}
...
async fn test_query_capabilities_no_socket() {
    ...
    assert!(capabilities.is_empty(), "Should return empty capabilities for unreachable primal");
}
...
async fn test_agnostic_capability_discovery() {
    ...
    let ready1 = start_mock_primal(beardog_path.clone(), "beardog", vec!["security", "crypto"]).await;
    let ready2 = start_mock_primal(songbird_path.clone(), "songbird", vec!["discovery", "mesh"]).await;
    let ready3 = start_mock_primal(custom_path.clone(), "my-custom-primal", vec!["ai", "ml", "inference"]).await;
    ...
    match tokio::time::timeout(
        ...
        Ok(Ok(_)) => {
            if let Ok(response) = serde_json::from_str::<serde_json::Value>(&response_line) {
                ...
                let status = response["result"]["status"].as_str().unwrap_or("healthy").to_string();
