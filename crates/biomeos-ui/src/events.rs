...
impl EventDispatcher {
    ...
    pub async fn dispatch_with_context(&self, _event: UIEvent, context: EventContext) -> Result<(), EventError> {
        // Update metrics
        {
            let mut stats = self.stats.write().await;
            stats.total_events += 1;
            *stats.events_by_priority.entry(context.priority).or_insert(0) += 1;
        }
        
        tracing::debug!("Dispatched event {} with priority {:?}", context.id, context.priority);
        Ok(())
    }
