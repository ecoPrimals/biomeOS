...
use crate::{BiomeManifest, Health, ResourceMetrics, PrimalCapability};
...
impl ManifestEditorComponent {
    ...
    pub fn save(&mut self) -> Result<(), crate::BiomeError> {
        ...
        let manifest = biomeos_manifest::BiomeManifestProcessor::load_from_yaml(&self.yaml_content)?;
        ...
        match data {
            ComponentData::Manifest(manifest) => {
                ...
                if let Some(manifest_ref) = &self.current_manifest {
                    if let Ok(yaml) = biomeos_manifest::BiomeManifestProcessor::save_to_yaml(manifest_ref) {
...
impl ComponentRegistry {
    ...
    pub fn active_components(&self) -> Vec<&dyn UIComponent> {
        self.components
            .values()
            .filter_map(|c| if c.is_active() { Some(c.as_ref()) } else { None })
            .collect()
    }
}
