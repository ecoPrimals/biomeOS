...
impl IsolationLevel {
    ...
    pub fn allows_auto_approval(&self) -> bool {
        matches!(self, IsolationLevel::None | IsolationLevel::Low | IsolationLevel::Medium)
    }
}
...
impl SubFederation {
    ...
    pub fn has_capability(&self, node_id: &str, capability: &Capability) -> bool {
        ...
        if !self.is_member(node_id) {
            debug!("Node {} is not a member of sub-federation {}", node_id, self.name);
            return false;
        }
...
impl SubFederationManager {
    ...
    pub fn new(config_dir: PathBuf) -> Self {
        info!("Initializing sub-federation manager at: {}", config_dir.display());
        Self {
            config_dir,
    ...
    pub async fn create(
        ...
        let subfed = SubFederation::new(name.clone(), parent_family, members, capabilities, isolation_level);
    ...
    pub async fn remove_member(&mut self, subfed_name: &str, node_id: &str) -> FederationResult<()> {
        {
            let subfed = self
                .sub_federations
                .get_mut(subfed_name)
        ...
        info!("Removed node {} from sub-federation {}", node_id, subfed_name);
        Ok(())
    }
}
