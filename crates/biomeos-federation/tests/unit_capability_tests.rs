...
fn test_capability_set_has_all() {
    ...
    let set2 = CapabilitySet::from_vec(vec![
        Capability::Storage,
        Capability::Compute,
    ]);
    
    let set3 = CapabilitySet::from_vec(vec![
        Capability::Storage,
        Capability::Voice,
    ]);
    
    assert!(set1.has_all(&set2), "set1 should have all capabilities from set2");
...
fn test_capability_custom_variants() {
    ...
    assert!(set.has(&cap1));
    assert!(set.has(&cap2));
    assert_eq!(set.all().len(), 2, "Same custom capability should not duplicate");
}
