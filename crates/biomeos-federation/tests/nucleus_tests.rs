...
async fn test_nucleus_with_clients() {
    ...
    let primals = nucleus.all();
    assert_eq!(primals.len(), 0, "NUCLEUS with no clients should have no primals");
}
...
async fn test_insecure_discovery() {
    ...
    assert!(result.is_ok(), "Insecure discovery should not error: {:?}", result);
...
async fn test_selection_by_capability() {
    ...
    assert!(found.is_some(), "Should find primal with encryption capability");
...
async fn test_selection_by_family() {
    ...
    let found = nucleus.get(SelectionCriteria::ByFamily("nat0".to_string()));
    
    assert!(found.is_some(), "Should find primal in nat0 family");
    assert_eq!(found.unwrap().family_id, Some("nat0".to_string()));
}
...
async fn test_selection_min_trust_level() {
    ...
    let low_trust = create_mock_verified_primal(
        "unknown-primal",
        "node-unknown",
        vec![],
        TrustLevel::Basic,
...
async fn test_multiple_instances() {
    ...
    let beardog1 = create_mock_verified_primal(
        "beardog",
        "node-alpha",
        vec!["security"],
        TrustLevel::High,
...
async fn test_selection_by_socket() {
    ...
    assert!(found.is_none(), "Should not find primal without proper endpoint injection");
}
