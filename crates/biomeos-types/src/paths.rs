...
pub enum PathError {
    ...
    CreateDirFailed { path: String, source: std::io::Error },
...
impl SystemPaths {
    ...
    pub fn genetic_seed(&self, family_id: &str) -> PathBuf {
        self.data_dir.join("seeds").join(format!("{}.seed", family_id))
    }
    ...
    pub fn log_file(&self, service_name: &str) -> PathBuf {
        self.state_dir.join("logs").join(format!("{}.log", service_name))
    }
