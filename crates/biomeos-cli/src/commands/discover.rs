...
use super::utils::{create_spinner, parse_capabilities, display_results};
...
pub async fn handle_discover(
    ...
    let discovery_result = match method {
        ...
        DiscoveryMethod::Multicast => {
            manager.discover_via_multicast().await?
        }
        DiscoveryMethod::RegistryBased => {
            if let Some(url) = registry {
                manager.discover_registry(&url).await?
            } else {
                return Err(anyhow::anyhow!("Registry URL required for registry discovery"));
            }
        }
    ...
    result.insert("method".to_string(), serde_json::json!(format!("{:?}", method)));
    result.insert("endpoints".to_string(), serde_json::json!(discovery_result));
    result.insert("count".to_string(), serde_json::json!(discovery_result.len()));
...
async fn display_discovery_results(
    ...
    for (service_id, service_info) in results {
        ...
        if detailed {
            ...
            if let Some(resource_usage) = service_info.get("resource_usage") {
                println!("   Resources: {}", serde_json::to_string_pretty(resource_usage)?);
            }
        }
    ...
    println!("📊 Summary: {}/{} services healthy", healthy_count, results.len());
