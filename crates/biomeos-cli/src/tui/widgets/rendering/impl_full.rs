...
//! Ratatui widget rendering (see parent `mod.rs` for module docs).

use super::WidgetRenderer;
use crate::tui::types::{DashboardState, TabId};
use crate::tui::widgets::formatting::{
    api_status_to_icon, api_status_to_str, compute_scroll_offset, compute_top_capabilities,
    confidence_bar, deployment_event_type_to_str, deployment_phase_icon, event_type_to_icon,
    format_ecosystem_header_text, format_performance_lines, format_primal_list_items_data,
    format_resource_usage_lines, health_status_icon, health_to_category_str, health_to_color,
    log_level_to_icon, log_level_to_str, role_to_prefix, role_to_str, service_status_icon,
    severity_to_icon, severity_to_str,
};
use ratatui::{
    Frame,
