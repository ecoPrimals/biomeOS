...
use crate::tui::types::{
    AiRole, DashboardState, DeploymentPhase, InsightSeverity, LogLevel, ServiceStatus, TabId,
};
...
impl WidgetRenderer {
    ...
    fn render_primal_status(f: &mut Frame, area: Rect, state: &DashboardState) {
        ...
        // Left: Primal list
        let primal_items: Vec<ListItem> = state
            .primal_states
            .iter()
            .map(|(id, primal_state)| {
                ...
                ListItem::new(format!(
                    "{} {} ({})",
                    health_icon, primal_state.metadata.name, id
                ))
            })
            .collect();
    ...
    fn render_performance_metrics(f: &mut Frame, area: Rect, state: &DashboardState) {
        ...
        for (id, primal_state) in &state.primal_states {
            ...
            perf_lines.push(Line::from(format!(
                "  RPS: {:.1}",
                primal_state.metrics.requests_per_second
            )));
            perf_lines.push(Line::from(format!(
                "  Response: {:?}",
                primal_state.metrics.average_response_time
            )));
            perf_lines.push(Line::from(format!(
                "  Error Rate: {:.2}%",
                primal_state.metrics.error_rate * 100.0
            )));
