...
pub enum UIEvent {
    ...
    ConfigurationUpdated { content: String, is_valid: bool },
}
...
impl UIController {
    ...
    pub async fn save_biome_config(&self, manifest: &BiomeManifest, file_name: &str) -> Result<()> {
        ...
        self.backend
            .update_yaml_content(file_name, &yaml_content)
            .await?;

        // Send event
        self.event_sender
            .send(UIEvent::ConfigurationUpdated {
                content: yaml_content,
                is_valid: true,
            })
            .map_err(|_| {
                BiomeError::internal_error("Failed to send configuration event", None::<String>)
            })?;
                ...
                if let Ok(health_report) = self.convert_status_to_health_report(status).await {
                    *self.health_cache.write().await = Some(health_report.clone());
                    ...
                    let _ = self.event_sender.send(UIEvent::SystemStatusUpdated {
                        ...
                        metrics: health_report.metrics.resources.unwrap_or({
                            ResourceMetrics {
                                cpu_usage: Some(0.0),
    ...
    async fn convert_status_to_health_report(
        &self,
        status: biomeos_core::integration::SystemStatus,
    ) -> Result<HealthReport> {
        use biomeos_types::{health::HealthMetrics, HealthSubject, HealthSubjectType};
