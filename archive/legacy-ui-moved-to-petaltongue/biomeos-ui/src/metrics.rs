...
use crate::{Health, ResourceMetrics};
...
impl TimeSeriesMetrics {
    ...
    pub fn data_points_in_range(
        &self,
        start: DateTime<Utc>,
        end: DateTime<Utc>,
    ) -> Vec<(DateTime<Utc>, ResourceMetrics)> {
        self.data_points
            .iter()
    ...
    pub fn average(&self) -> Option<ResourceMetrics> {
        ...
        Some(ResourceMetrics {
            ...
            network_io: self
                .data_points
                .back()
                .and_then(|(_, m)| m.network_io.clone()),
        })
    }
            ...
            || disk_usage >= self.disk_critical_threshold
        {
            ...
            if cpu_usage >= self.cpu_critical_threshold {
                issues.push(biomeos_types::HealthIssue {
                    ...
                    detected_at: Utc::now(),
                    details: [("cpu_usage".to_string(), serde_json::json!(cpu_usage))]
                        .into_iter()
                        .collect(),
                    remediation: vec![
            ...
            if memory_usage >= self.memory_critical_threshold {
                issues.push(biomeos_types::HealthIssue {
                    ...
                    detected_at: Utc::now(),
                    details: [("memory_usage".to_string(), serde_json::json!(memory_usage))]
                        .into_iter()
                        .collect(),
                    remediation: vec![
                        biomeos_types::RemediationAction {
            ...
            if disk_usage >= self.disk_critical_threshold {
                issues.push(biomeos_types::HealthIssue {
                    ...
                    detected_at: Utc::now(),
                    details: [("disk_usage".to_string(), serde_json::json!(disk_usage))]
                        .into_iter()
                        .collect(),
                    remediation: vec![
                        biomeos_types::RemediationAction {
