...
use biomeos_core::integration::{LiveService, NetworkInterface, SystemStatus};
...
impl LiveBackend {
    ...
    pub async fn new() -> Result<Arc<Self>> {
        ...
        let integration_service = Arc::new(LiveService::new().await?);
    ...
    async fn start_monitoring_tasks(&self) {
        ...
        loop {
            ...
            if let Ok(status) = self.integration_service.get_system_status().await {
                ...
                if self
                    .event_sender
                    .send(BackendEvent::SystemStatusUpdated(status))
                    .is_err()
                {
    ...
    pub async fn update_yaml_content(&self, file_name: &str, content: &str) -> Result<()> {
        ...
        if self
            .event_sender
            .send(BackendEvent::YamlFileChanged(file_name.to_string()))
            .is_err()
        {
