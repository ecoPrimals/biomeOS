...
use biomeos_ui::{DashboardMetrics, LiveBackend};
...
impl BiomeOSApi {
    ...
    /// Get installation status (LIVE - checks actual system state)
    pub async fn get_installation_status(&self) -> Result<InstallationStatus> {
        if !self.is_connected().await {
            return Err(anyhow::anyhow!("API not connected to live backend"));
        }

        info!("🔍 API: Checking installation status (LIVE)");

        // Get real system status from backend
        // For now, return a basic status since the backend doesn't expose the method we need
    ...
    /// Get niche manager data (LIVE - reads from filesystem and registry)
    pub async fn get_niches(&self) -> Result<Vec<NicheInfo>> {
        if !self.is_connected().await {
            return Err(anyhow::anyhow!("API not connected to live backend"));
        }

        info!("📦 API: Loading niches from filesystem (LIVE)");

        // Get YAML files from niche directories
        match self.get_yaml_files().await {
            Ok(files) => {
                let mut niches = Vec::new();

                // Parse niche manifest files
                for (filename, content) in files.iter() {
                    // Only process niche manifest files
                    if filename.contains("niche") && filename.ends_with(".yaml") {
                        // Parse basic niche info from YAML content
                        match serde_yaml::from_str::<serde_yaml::Value>(&content) {
                            Ok(yaml) => {
                                let id = yaml["id"]
                                    .as_str()
                                    .unwrap_or_else(|| filename.trim_end_matches(".yaml"))
                                    .to_string();
                                ...
                                let category =
                                    yaml["category"].as_str().unwrap_or("General").to_string();

                                let status =
                                    yaml["status"].as_str().unwrap_or("Active").to_string();
                            ...
                            Err(e) => {
                                warn!("Failed to parse niche manifest {}: {}", filename, e);
                            }
                        }
                    }
                }

                info!("✅ Loaded {} niches from filesystem", niches.len());
                Ok(niches)
            }
            Err(e) => {
                self.set_error(format!("Failed to get niches: {}", e)).await;
                Err(e)
            }
        }
    }
