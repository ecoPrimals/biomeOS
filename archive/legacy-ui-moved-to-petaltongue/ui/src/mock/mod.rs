...
//!
//! Mock data is ONLY for development and testing. Production builds should:
//! 1. Use `--release` flag (disables debug_assertions)
//! 2. Set `BIOMEOS_MOCK_DISABLED=1` environment variable
//! 3. Connect to real Songbird discovery for live primal data
//!
//! When mock data is disabled, UI components MUST fetch data from:
//! - Songbird discovery for primal endpoints
//! - NestGate for storage/niche information
//! - Toadstool for runtime/service information
...
/// Mock data enabled only in debug builds unless explicitly disabled
/// Set BIOMEOS_MOCK_DISABLED=1 in environment to disable even in debug builds
pub const MOCK_DATA_ENABLED: bool = cfg!(debug_assertions);

/// Central mock data provider that coordinates all mock data
///
/// # Production Usage
///
/// In production, always check `is_enabled()` before using mock data.
/// If disabled, fetch real data from the primal ecosystem.
pub struct UnifiedMockProvider;

impl UnifiedMockProvider {
    /// Check if mock data is enabled
    ///
    /// Returns false in release builds or when BIOMEOS_MOCK_DISABLED is set.
    /// UI components should check this before using mock providers.
    pub fn is_enabled() -> bool {
        if std::env::var("BIOMEOS_MOCK_DISABLED").is_ok() {
            return false;
        }
        MOCK_DATA_ENABLED
    }
