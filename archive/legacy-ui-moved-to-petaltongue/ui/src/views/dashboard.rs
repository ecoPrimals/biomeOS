    ...
    async fn collect_system_metrics(&self) -> Option<LiveSystemMetrics> {
        ...
        Some(LiveSystemMetrics {
            timestamp: std::time::SystemTime::now()
                ...
                .ok()?
                .as_secs_f64(),
            cpu_usage_percent: (system_status.resource_usage.cpu_usage.unwrap_or(0.0) * 100.0)
                as f32,
            memory_usage_percent: (system_status.resource_usage.memory_usage.unwrap_or(0.0) * 100.0)
                as f32,
            memory_used_gb: (system_status.resource_usage.memory_usage.unwrap_or(0.0) * 16.0)
                as f32, // Estimate
            memory_total_gb: 16.0, // Estimate - could be detected
            disk_usage_percent: (system_status.resource_usage.disk_usage.unwrap_or(0.0) * 100.0)
                as f32,
            ...
            network_rx_mbps: system_status
                .resource_usage
                .network_io
                .as_ref()
                .map_or(0.0, |n| n.bytes_in_per_sec / 1_000_000.0)
                as f32,
            network_tx_mbps: system_status
                .resource_usage
                .network_io
                .as_ref()
                .map_or(0.0, |n| n.bytes_out_per_sec / 1_000_000.0)
                as f32,
