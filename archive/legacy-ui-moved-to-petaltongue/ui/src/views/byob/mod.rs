...
mod tests {
    ...
    fn test_universal_primal_system() {
        ...
        for primals in test_cases {
            byob_view.selected_primals = primals.into_iter().map(|s| s.to_string()).collect();

            // Verify the system can handle any primal combination
            assert!(!byob_view.selected_primals.is_empty());

            // Test capability detection
            let capabilities = byob_view.get_capabilities_for_primals(&byob_view.selected_primals);
            assert!(
    ...
    fn test_yaml_template_loading() {
        ...
        for template in templates {
            ...
            assert!(!template.required_capabilities.is_empty() || !template.preferred_primals.is_empty());
        }
    }
