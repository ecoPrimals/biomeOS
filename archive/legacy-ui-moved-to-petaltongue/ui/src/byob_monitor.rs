...
use crate::views::byob::{
    DeploymentInfo, DeploymentStatus, HealthCheck, ResourceQuota, ResourceUsage, ServiceInfo,
    ServiceStatus, TeamInfo,
};
...
impl ByobMonitor {
        ...
        for entry in fs::read_dir(teams_path)? {
            ...
            if entry.path().extension().and_then(|s| s.to_str()) == Some("yaml") {
                ...
                if let Ok(team_config) = serde_yaml::from_str::<TeamConfig>(&content) {
                    let mut team_info =
                        TeamInfo::basic(team_config.id, team_config.name, team_config.description);
    ...
    fn read_deployments_from_filesystem(
        &self,
        ...
        for entry in fs::read_dir(deployments_path)? {
            ...
            if entry.path().extension().and_then(|s| s.to_str()) == Some("yaml") {
                ...
                if let Ok(deployment_config) = serde_yaml::from_str::<DeploymentConfig>(&content) {
                    let services: Vec<ServiceInfo> = deployment_config
                        .services
                        .into_iter()
                        .map(|s| {
                            ServiceInfo {
            ...
            for line in processes.lines() {
                if line.contains("biomeos-team-") {
                    ...
                    if let Some(team_name) = self.extract_team_from_process(line) {
                        ...
                        team_info.workspace_url = Some(format!(
                            "https://{}.biomeos.local",
    ...
    fn calculate_deployment_health(&self, services: &[ServiceInfo]) -> f64 {
        ...
        let healthy_count = services
            .iter()
            .filter(|s| matches!(s.health_check.status, Health::Healthy))
            .count();
