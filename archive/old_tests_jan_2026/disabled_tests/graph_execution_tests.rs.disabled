...
use biomeos_atomic_deploy::neural_executor::{
    ExecutionReport, GraphExecutor as NeuralGraphExecutor, NodeStatus,
};
...
async fn test_deep_nested_circular_dependency() {
    ...
    assert!(
        result.is_err(),
        "Should detect deep nested circular dependency"
    );
    if let Err(e) = result {
...
async fn test_timeout_with_many_phases() {
    ...
    assert!(
        result.is_ok(),
        "Multi-phase graph should complete within timeout"
    );
}
...
async fn test_single_node_graph() {
    ...
    if let Err(e) = &result {
        assert!(
            ...
            "Single node should not have cycle"
        );
    }
}
...
async fn test_missing_dependency_handling() {
    ...
    let nodes = vec![create_node("node_a", "test", vec!["nonexistent_node"])];
...
async fn test_error_propagation() {
    ...
    let nodes = vec![create_node("node_a", "nonexistent_node_type", vec![])];
