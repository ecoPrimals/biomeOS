...
## v4.49 (2026-07-30) — Wave 155k: P2 Divergence Fixes

### Capability Wipe Cycle Fix (P2)
- Root cause: `prune_stale_registrations()` racing with discovery sweep — single failed
  health probe unregistered ALL capabilities for a primal during socket contention
- Fix: 3-strike consecutive failure threshold. `prune_strikes` counter per primal.
  Single successful health check resets. Eliminates 654→0→187→654 oscillation.

### Socket Path Documentation Fix (P2)
- Stale doc comments in `neural-api-client-sync` updated from `biomeos/` to `membrane/`

### Dark Forest 403 Clarification (P2 — docs only)
- HTTP 403 on non-/health is intentional sovereign mode behavior
- Inter-primal communication uses Neural API UDS (capabilities.list is Public)
