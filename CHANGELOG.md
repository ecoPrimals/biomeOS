...
- Root cause: `check_endpoint_health` and `call_primal_health` required `"healthy": true`
  in JSON-RPC response body. Primals returning `{"status":"alive"}` or any non-standard
  format were falsely marked dead → capabilities pruned after 3 strikes (~180s).
- Fix: any successful `call_btsp()` response = alive. `call_btsp` already rejects
  connection failures, timeouts, and JSON-RPC error responses. `Ok(_)` proves liveness.
- `check_endpoint_health`: `unwrap_or(false)` → unconditional `true` on `Ok`
- `quick_health_check`: aligned (already `unwrap_or(true)`, simplified to `Ok(_) => true`)
- `call_primal_health`: removed body parsing, `Ok(_)` from `call_btsp` = alive
- `node_handlers::health_check`: tolerant fallback — respects explicit `"healthy": false`
  but treats missing field as alive (backward compatible)

### Binary Path Retention Fix (P2)
- Root cause: auto-discovery (`discovery_init.rs`) registered primals with
  `binary_path: None`. When primals crashed, resurrection failed with
  "No deployment node or binary path — cannot resurrect."
- Fix: auto-discovery now probes plasmidBin search directories (`probe_binary()`) during
  both UDS and TCP registration. If binary found, passes `binary_path` to
  `lifecycle.register` which uses `register_primal_binary()` for resurrection support.
- Extended `LifecycleHandler::register` to accept optional `binary_path` parameter.
  When present, uses `register_primal_binary` instead of `register_primal`.
