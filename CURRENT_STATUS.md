...
**Updated**: March 28, 2026 (v2.70: multi-transport IPC evolution — `RegisteredCapability`, `DiscoveredPrimal`, `DiscoveredAtomic` evolved from `PathBuf` to `TransportEndpoint` (Unix/abstract/TCP/HTTP), `forward_request` routes via any transport, health checks transport-aware (no `Path::exists()` for abstract/TCP), `capability.register` parses transport strings, `TransportEndpoint` gains `Serialize`/`Deserialize`, 5 new multi-transport tests, all primalSpring P0+P1 gaps resolved)
**Version**: 2.70
...
| **Tests Passing** | 7,165 lib + bin + doc + proptest (0 failures, ~135 ignored hardware-dependent — run with `--ignored --test-threads=1`) |
