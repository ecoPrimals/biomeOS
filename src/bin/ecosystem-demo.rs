...
use biomeos_core::{
    ecosystem_integration::{
        EcosystemCoordinator, EcosystemServiceRegistry, EcosystemServiceRegistration,
        EcosystemEndpoints, EcosystemCapabilities, EcosystemSecurity, 
        ResourceRequirements, HealthCheckConfig, InMemoryMessageBus,
    },
...
impl EcosystemDemo {
    ...
    fn load_default_config() -> EcosystemDemoConfig {
        ...
        primals.insert("squirrel".to_string(), PrimalConfig {
        ...
        demo_scenarios.insert("full-stack".to_string(), DemoScenario {
            ...
            steps: vec![
                ...
                DemoStep {
                    ...
                    target_primals: vec!["songbird".to_string(), "nestgate".to_string(), "toadstool".to_string()],
    ...
    async fn initialize_ecosystem(&mut self, name: String) -> BiomeResult<()> {
        ...
        let primals_to_register: Vec<_> = self.config.primals.iter()
    ...
    async fn register_primal(&mut self, primal_type: String, endpoint: String) -> BiomeResult<()> {
        ...
        self.coordinator.service_registry.register_service(registration).await?;
        
        info!("✅ {} successfully registered", primal_type);
        Ok(())
    }
    
    fn create_primal_registration(&self, primal_type: &str, endpoint: &str) -> BiomeResult<EcosystemServiceRegistration> {
        let (capabilities, metadata) = match primal_type {
            ...
            "toadstool" => (
                ...
                },
                {
                    ...
                    meta.insert("runtimes".to_string(), "container,native,wasm,python".to_string());
                    meta
                }
            ),
            _ => return Err(BiomeError::InvalidInput(format!("Unknown primal type: {}", primal_type))),
        };
    ...
    async fn deploy_biome(&self, manifest_path: PathBuf, watch: bool) -> BiomeResult<()> {
        ...
        self.simulate_coordinated_deployment(&manifest_content, watch).await?;
        
        info!("✅ Biome deployment complete!");
        Ok(())
    }
    
    async fn simulate_coordinated_deployment(&self, _manifest: &str, watch: bool) -> BiomeResult<()> {
        let steps = vec![
    ...
    async fn show_ecosystem_status(&self, detailed: bool, watch: bool) -> BiomeResult<()> {
        loop {
            ...
            if detailed {
                ...
                let songbird_services = self.coordinator.service_registry.get_services_by_type("songbird").await;
                let nestgate_services = self.coordinator.service_registry.get_services_by_type("nestgate").await;
                let toadstool_services = self.coordinator.service_registry.get_services_by_type("toadstool").await;
    ...
    async fn run_health_checks(&self, comprehensive: bool) -> BiomeResult<()> {
        ...
        let health = self.coordinator.service_registry.check_ecosystem_health().await?;
        
        info!("📊 Health Check Results:");
        info!("═══════════════════════");
        info!("Overall: {:?}", health.overall_health);
        info!("Services: {}/{} healthy", health.healthy_services, health.total_services);
    ...
    async fn run_demo_scenario(&self, scenario: String) -> BiomeResult<()> {
        ...
        let demo_scenario = self.config.demo_scenarios.get(&scenario)
            .ok_or_else(|| BiomeError::InvalidInput(format!("Unknown scenario: {}", scenario)))?;
    ...
    async fn list_primals(&self) -> BiomeResult<()> {
        ...
        for (primal_type, config) in &self.config.primals {
            ...
            info!("{} {} {} - {}", status_emoji, auto_register, primal_type, config.endpoint);
        }
        ...
        for from_primal in &primals {
            ...
            for to_primal in &primals {
                let connection = if from_primal == to_primal {
                    "    -    "
                } else if self.config.primals.get(*from_primal).map_or(false, |c| c.enabled) &&
                          self.config.primals.get(*to_primal).map_or(false, |c| c.enabled) {
                    "    ✅    "
                } else if from_primal == &"beardog" || from_primal == &"squirrel" ||
                          to_primal == &"beardog" || to_primal == &"squirrel" {
                    "    🔄    "
                } else {
                    "    ❌    "
                };
...
async fn main() -> BiomeResult<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(format!("ecosystem_demo={},biomeos_core={}", log_level, log_level))
        .init();
    ...
    match args.command {
        ...
        Commands::Primals { action } => {
            match action {
                PrimalCommands::List => {
                    demo.list_primals().await?;
                }
                PrimalCommands::Register { primal_type, endpoint } => {
...
mod tests {
    ...
    async fn test_ecosystem_initialization() {
        ...
        demo.initialize_ecosystem("test-ecosystem".to_string()).await.unwrap();
    ...
    async fn test_primal_registration() {
        ...
        let registration = demo.create_primal_registration(
            "songbird", 
            "http://localhost:8080"
        ).unwrap();
