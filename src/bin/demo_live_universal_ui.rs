...
impl LiveUniversalUIDemo {
    ...
    async fn check_primal_health(&self, name: &str, endpoint: &str) -> Result<PrimalInfo> {
        ...
        for health_endpoint in health_endpoints {
            match tokio::time::timeout(timeout_duration, self.client.get(&health_endpoint).send())
                .await
            {
                Ok(Ok(response)) if response.status().is_success() => {
                    ...
                    let capabilities = self
                        .get_primal_capabilities(endpoint)
                        .await
    ...
    async fn demonstrate_universal_ui(&self) -> Result<()> {
        ...
        info!(
            "🎯 Demonstrating Universal UI with {} real primals",
            self.discovered_primals.len()
        );
    ...
    async fn show_system_overview(&self) -> Result<()> {
        ...
        for primal in &self.discovered_primals {
            let health_icon = if primal.health == "healthy" {
                "✅"
            } else {
                "❌"
            };
            println!(
                "│ {:<15} │ {:<43} │ {} {:>6} │",
                primal.name, primal.endpoint, health_icon, primal.health
            );
        }
    ...
    async fn test_basic_operations(&self) -> Result<()> {
        ...
        for primal in &self.discovered_primals {
            ...
            match self
                .client
                .get(format!("{}/health", primal.endpoint))
                .send()
                .await
            {
            ...
            match self
                .client
                .get(format!("{}/api/v1/metrics", primal.endpoint))
                .send()
                .await
            {
                Ok(response) if response.status().is_success() => {
    ...
    async fn simulate_ai_assistant(&self) -> Result<()> {
        ...
        let ai_scenarios = vec![
            (
                ...
                "All discovered primals are healthy and ready",
            ),
            (
                "Deploy a web application",
                "I'll coordinate the available primals for deployment",
            ),
            (
                "Scale services based on load",
                "I'll analyze current metrics and scale appropriately",
            ),
            (
                "Optimize resource allocation",
                "I'll rebalance resources across available primals",
            ),
        ];
    ...
    async fn show_summary(&self) -> Result<()> {
        ...
        println!(
            "  ✅ Primal Discovery: {} primals found",
            self.discovered_primals.len()
        );
