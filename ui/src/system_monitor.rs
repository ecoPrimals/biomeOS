...
impl SystemMonitor {
    ...
    fn get_cpu_usage(&mut self) -> Result<f32, Box<dyn std::error::Error>> {
        ...
        let cpu_usage = if let Some(last_times) = &self.last_cpu_times {
            let total_diff = (current_times.user + current_times.nice + current_times.system + 
                           ...
                           (last_times.user + last_times.nice + last_times.system + 
                            last_times.idle + last_times.iowait + last_times.irq + 
                            last_times.softirq + last_times.steal);
    ...
    fn get_network_io(&mut self) -> Result<(f32, f32), Box<dyn std::error::Error>> {
        ...
        let (rx_mbps, tx_mbps) = if time_diff > 0.0 && !self.last_network_stats.is_empty() {
            let last_total_rx = self.last_network_stats.values().map(|s| s.rx_bytes).sum::<u64>();
            let last_total_tx = self.last_network_stats.values().map(|s| s.tx_bytes).sum::<u64>();
        ...
        self.last_network_stats.insert("total".to_string(), NetworkStats {
