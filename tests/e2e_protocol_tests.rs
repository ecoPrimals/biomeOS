...
"#;
    ...
    let config: biomeos_core::TowerConfig = toml::from_str(&config_str).expect("Failed to parse config");
...
"#;
    ...
    let prod_config_str = fs::read_to_string(&prod_config_path).expect("Failed to read prod config");
    let prod_config: biomeos_core::TowerConfig = toml::from_str(&prod_config_str).expect("Failed to parse prod config");
...
"#;
    ...
    let dev_config: biomeos_core::TowerConfig = toml::from_str(&dev_config_str).expect("Failed to parse dev config");
...
"#;
    ...
    assert_eq!(config.primals[1].protocol, Some("tarpc".to_string()));
    assert!(config.primals[1].env.get("SECURITY_ENDPOINT")
        .unwrap()
        .starts_with("tarpc+unix://"));
    
    // ToadStool: JSON-RPC with jsonrpc+unix:// URL
    assert_eq!(config.primals[2].protocol, Some("jsonrpc".to_string()));
    assert!(config.primals[2].env.get("WORKLOAD_ENDPOINT")
        .unwrap()
...
"#;
    ...
    // BearDog
    assert_eq!(config.primals[0].provides, vec!["Security", "Encryption", "Trust"]);
    assert_eq!(config.primals[0].protocol, Some("tarpc".to_string()));
    assert_eq!(
        config.primals[0].env.get("BEARDOG_FAMILY_ID"),
        Some(&"nat0".to_string())
