...
use biomeos_ui::{
    ...
    PrimalUIConfig, UIFeatures, SystemStatus, UserInput, UIResponse
};
...
async fn load_demo_config() -> Result<UniversalUIConfig> {
    ...
    config.primal_endpoints.insert("toadstool".to_string(), "http://localhost:8084".to_string());
    config.primal_endpoints.insert("beardog".to_string(), "http://localhost:9000".to_string());
    config.primal_endpoints.insert("squirrel".to_string(), "http://localhost:5000".to_string());
...
fn add_custom_primal_configs(config: &mut UniversalUIConfig) {
    use biomeos_ui::universal_ui::*;
    
    // Custom AI Primal
    config.custom_primals.insert("custom_ai".to_string(), CustomPrimalConfig {
            ...
            dashboard_widgets: vec![
                WidgetConfig {
                    ...
                    display_config: {
                        ...
                        config.insert("chart_type".to_string(), serde_json::Value::String("line".to_string()));
                        config.insert("metrics".to_string(), serde_json::json!(["requests_per_second", "latency_ms"]));
                        config
                    },
                },
                WidgetConfig {
                    ...
                    display_config: {
                        let mut config = HashMap::new();
                        config.insert("show_model_list".to_string(), serde_json::Value::Bool(true));
                        config.insert("show_gpu_usage".to_string(), serde_json::Value::Bool(true));
                        config
                    },
                },
            ],
    ...
    config.custom_primals.insert("custom_storage".to_string(), CustomPrimalConfig {
        endpoint: "http://localhost:7001".to_string(),
        capabilities: vec!["storage".to_string(), "backup".to_string(), "replication".to_string()],
    ...
    config.custom_primals.insert("gpu_compute".to_string(), CustomPrimalConfig {
        endpoint: "http://localhost:7002".to_string(),
            ...
            dashboard_widgets: vec![
                WidgetConfig {
                    ...
                    display_config: {
                        ...
                        config.insert("show_memory_usage".to_string(), serde_json::Value::Bool(true));
                        config
                    },
                },
            ],
    ...
    match ui_manager.get_system_status().await {
        Ok(status) => {
            ...
            if let Some(last_discovery) = status.last_discovery {
                println!("   🕐 Last Discovery: {}", last_discovery.format("%Y-%m-%d %H:%M:%S UTC"));
            }
...
async fn demo_primal_discovery(ui_manager: &UniversalUIManager) -> Result<()> {
    println!("\n🔍 Scenario 2: Dynamic Primal Discovery");
    println!("──────────────────────────────────────");
    ...
    let discovered_primals = vec![
        ("songbird", "Orchestration", "healthy", vec!["orchestration", "coordination"]),
        ("nestgate", "Storage", "healthy", vec!["storage", "zfs", "backup"]),
        ("toadstool", "Compute", "healthy", vec!["compute", "wasm", "containers"]),
        ("beardog", "Security", "healthy", vec!["security", "encryption", "tunnel"]),
        ("squirrel", "AI/MCP", "degraded", vec!["ai", "mcp", "automation"]),
        ("custom_ai", "Custom AI", "healthy", vec!["ai", "ml", "inference"]),
        ("custom_storage", "Custom Storage", "healthy", vec!["storage", "backup", "replication"]),
        ("gpu_compute", "GPU Cluster", "healthy", vec!["compute", "gpu", "hpc"]),
    ];
    
    println!("✅ Found {} primals in the ecosystem:", discovered_primals.len());
...
async fn demo_multi_primal_coordination(ui_manager: &UniversalUIManager) -> Result<()> {
    println!("\n🤝 Scenario 3: Multi-Primal Coordination");
    println!("───────────────────────────────────────");
...
async fn demo_custom_primal_integration(ui_manager: &UniversalUIManager) -> Result<()> {
    println!("\n🔌 Scenario 4: Custom Primal Integration");
    println!("──────────────────────────────────────");
    ...
    let custom_primals = vec![
        ...
        ("custom_storage", "Storage Engine", "Creating encrypted backup"),
...
async fn demo_ai_assisted_operations(ui_manager: &UniversalUIManager) -> Result<()> {
    println!("\n🤖 Scenario 5: AI-Assisted Operations");
    println!("────────────────────────────────────");
    ...
    let ai_commands = vec![
        ("What's the status of all primals?", "All primals are healthy except squirrel which is degraded"),
        ("Deploy a web application", "I'll coordinate songbird, nestgate, and toadstool for your web app"),
        ("Optimize GPU utilization", "I'll work with gpu_compute to balance your workload"),
        ("Create encrypted backup", "I'll use custom_storage and beardog for secure backup"),
    ];
...
async fn demo_real_time_monitoring(ui_manager: &UniversalUIManager) -> Result<()> {
    println!("\n📊 Scenario 6: Real-time Monitoring");
    println!("──────────────────────────────────");
...
async fn demo_configuration_adaptability(ui_manager: &UniversalUIManager) -> Result<()> {
