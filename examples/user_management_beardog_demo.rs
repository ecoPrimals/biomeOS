...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let alice_id = user_manager.create_user_with_beardog(
        "alice",
        UserAuthMethod::Password { password: "secure_password".to_string() },
    ...
    let bob_id = user_manager.create_user_with_beardog(
        "bob",
        UserAuthMethod::Password { password: "business_password".to_string() },
    ...
    let charlie_id = user_manager.create_user_with_beardog(
        "charlie",
        UserAuthMethod::Password { password: "research_password".to_string() },
    ...
    println!("   Security Level: {}", session.beardog_context.security_level);
    println!("   Threat Score: {:.2}", session.beardog_context.threat_assessment_score);
    println!("   Compliance: {}", session.beardog_context.compliance_status);
    println!();
    ...
    let ssh_key_ref = user_manager.add_user_ssh_key(
        "alice",
    ...
    let api_key_ref = user_manager.generate_user_api_key(
        "alice",
        "alice_api_key",
    ...
    let genetic_auth_request = UserAuthRequest {
        username: "alice".to_string(),
        auth_method: UserAuthMethod::GeneticKey { key: genetic_key },
        client_ip: Some("192.168.1.100".to_string()),
        client_user_agent: Some("research-client/1.0".to_string()),
        metadata: HashMap::new(),
    };
    
    let genetic_session = user_manager.authenticate_with_beardog(genetic_auth_request).await?;
    println!("✅ Alice authenticated with genetic BearDog key");
    println!("   Session ID: {}", genetic_session.id);
    println!("   Security Level: {}", genetic_session.beardog_context.security_level);
    println!();
    ...
    for (username, user) in all_users.iter() {
        println!("  {} - {:?} - {}", 
            username, 
            user.access_level, 
