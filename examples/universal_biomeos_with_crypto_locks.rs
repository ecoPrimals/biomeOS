...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut manager = UniversalBiomeManager::new(config);
    
    // Initialize with different access levels
    println!("🏠 Power User Access (Sovereign Priority):");
    manager.initialize_partnership_access(power_user_key).await?;
    ...
    manager.initialize_partnership_access(small_business_key).await?;
    ...
    let installation_result = platform.install_biomeos(&manager.config.platform.deployment).await;
